...
def base2to10(binary)
	sum = 0
	binary.reverse.chars.each_with_index do |x, idx|
		sum += Integer(x) * 2 ** idx
	end
	sum
...
class Hash
	...
	def my_merge(hash, &prc)
		...
		self.each do |x, y|
			result[x] = hash[x] ? prc.call(x, y, hash[x]) : y
		end

		hash.each do |x, y|
			result[x] = y if result[x].nil?
		end
		result
...
def longest_palindrome(string)
	...
	string.length.downto(1).each do |idx|
		string.chars.each_cons(idx) do |subs|
			return subs.length if subs == subs.reverse && subs.length > 2
		end
	end
	false
